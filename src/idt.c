...
void initIdt() {
    ...
    // PIC chips (main vs secondary) (programmable interupt computer -  PIC)
    // One chip sits at 0x20 for cmd and 0x21 for data
    // 2nd sits at 0xA0 cmd and 0xA1 data

    outPortB(0x20, 0x11); // sends 11 to the cmd port of both chips
    outPortB(0xA0, 0x11); // starts chips up into init mode require 3 more cmds

    outPortB(0x21, 0x20); // sets up vector offsets for chips
    outPortB(0xA1, 0x28); // not sure what this does fully
    ...
    // setting up gates for interupts
...
unsigned char* exception_msgs[] = {"Divide By Zero",
                                   "Debug",
